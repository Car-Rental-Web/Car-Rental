...
  const totalPages = Math.ceil(renterData.length / itemsPerPage);
  ...
  // LOGIC: Helper to sort by Newest Date and Latest Time
  const sortByNewest = (data: DataRenterHistoryProps[]) => {
...
        .is("deleted_at", null)
        // LOGIC: Changed to Descending order
...
            setFilterRenterData((prev) => sortByNewest([...prev, newData]));
          } else if (eventType === "UPDATE") {
            const updatedData = payload.new as DataRenterHistoryProps;
            setFilterRenterData((prev) =>
              sortByNewest(prev.map((item) => (item.id === updatedData.id ? updatedData : item)))
            );
...
                <th className="p-4 text-[11px] font-bold uppercase tracking-wider text-slate-400 text-center">Location</th>
                <th className="p-4 text-[11px] font-bold uppercase tracking-wider text-slate-400 text-center">Total Price</th>
                <th className="p-4 text-[11px] font-bold uppercase tracking-wider text-slate-400 text-center">Downpayment</th>
                <th className="p-4 text-[11px] font-bold uppercase tracking-wider text-slate-400 text-center">Balance</th>
...
                      </div>
                    </td>
                    <td className="p-4 text-center"><span className="text-xs font-bold text-blue-600">{row.car_plate_number}</span></td>
                    <td className="p-4 text-center">
                      <div className="text-xs font-medium text-slate-700">
                        {formatDateandDay(row.start_date)} <span className="text-slate-300">|</span> {formatDateandDay(row.end_date)}
                        <div className="text-[10px] text-slate-400 mt-0.5">{to12Hour(row.start_time)} - {to12Hour(row.end_time)}</div>
                        <div className="text-xs text-red-500 text-left text-ellipsis"><strong className="text-black">Remarks:</strong> {row.remarks || ""}</div>
                      </div>
                    </td>
...
                    <td className="p-4 text-center text-xs font-black text-slate-700">{row.location}</td>
                    <td className="p-4 text-center text-xs font-black text-slate-700">{row.total_price_rent}</td>
                    <td className="p-4 text-center text-xs font-black text-slate-700">{row.downpayment}</td>
                    <td className="p-4 text-center text-xs font-black text-red-500">{row.remaining_balance}</td>
                    <td className="p-4">
                      <div className="flex justify-center">
                        <span className={`px-3 py-1 rounded-md text-[10px] font-bold uppercase tracking-tighter shadow-sm border ${row.status === "Completed" ? "bg-red-500 text-red-100 border-red-400" : row.status === "On Service" ? "bg-emerald-500 text-emerald-100 border-emerald-400" : row.status === "On Reservation" ? "bg-blue-500 text-blue-100 border-blue-400" : "bg-slate-50 text-slate-700 border-slate-200"}`}>{row.status}</span>
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="flex gap-2 justify-center">
                        <button onClick={() => handleAction("view", row)} className="cursor-pointer p-2 bg-white border border-slate-200 rounded-lg text-emerald-600 hover:bg-emerald-50 transition-all shadow-sm"><icons.openEye size={16} /></button>
                        <button onClick={() => handleAction("edit", row)} className="cursor-pointer p-2 bg-white border border-slate-200 rounded-lg text-blue-600 hover:bg-blue-50 transition-all shadow-sm"><icons.edit size={16} /></button>
                        <button onClick={(e) => { e.stopPropagation(); setSelectedData(row); setOpenDelete(true); }} className="cursor-pointer p-2 bg-white border border-slate-200 rounded-lg text-red-500 hover:bg-red-50 transition-all shadow-sm"><icons.trash size={16} /></button>
                      </div>
                    </td>
                  </tr>
                ))
              ) : (
                <tr><td colSpan={13} className="p-20 text-center text-slate-500 font-medium">No records found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
            <select value={itemsPerPage} onChange={(e) => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-xs font-bold text-slate-700 outline-none">
              {[5, 10, 15, 20].map((val) => (<option key={val} value={val}>{val}</option>))}
            </select>
          </div>
          <div className="h-4 w-px bg-slate-200 mx-2" />
          <p className="text-xs font-semibold text-slate-500">Results: <span className="text-slate-900">{renterData.length === 0 ? 0 : indexOfFirstItem + 1}-{Math.min(indexOfLastItem, renterData.length)}</span> of <span className="text-slate-900">{renterData.length}</span></p>
        </div>
        <div className="flex items-center gap-3">
          <button disabled={currentPage === 1} onClick={() => setCurrentPage((prev) => prev - 1)} className="px-4 py-2 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 disabled:opacity-40 transition-all shadow-sm cursor-pointer">Prev</button>
          <span className="text-xs font-bold text-blue-600 bg-blue-50 px-3 py-2 rounded-xl border border-blue-100">{currentPage} <span className="text-blue-300 mx-1">/</span> {totalPages || 1}</span>
          <button disabled={currentPage >= totalPages} onClick={() => setCurrentPage((prev) => prev + 1)} className="px-4 py-2 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 disabled:opacity-40 transition-all shadow-sm cursor-pointer">Next</button>
        </div>
      </div>

      <RenterForm open={openForm} mode={formMode} selectedData={selectedData} onClose={() => setOpenForm(false)} />
      {openDelete && selectedData && (
        <DeleteModal onClose={() => { setOpenDelete(false); setSelectedData(null); }} onClick={() => handleDelete(selectedData.id)} open={openDelete} />
      )}
    </div>
  );
};
